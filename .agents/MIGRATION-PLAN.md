...
**Last Updated:** 2026-06. 8.9 (catalog base) complete on master. 9.0 (daily MIGRATION-TODOS list). 9.1: Criterios Ambientales menu hygiene (0019 + 0018 NOTE close). 9.2: Proveedores (0020, basic CRUD via catalog base). 9.3: Equipos (0021 table+seed, core fields CRUD with overrides, correct routes per clientes pattern) + hygiene fix for the 9.2 Proveedores list routes (were pointing to Formulario ShowPage instead of Listado). 9.4: Acciones de Mejora (0022 table+seed for acciones_mejora, Pages/Mejora with custom overrides for descripcion/fecha/cerrada + multi-fields, correct routes, templates, full verify/playbook + TODOS refresh). All via plan-first on feature branch, full verify/playbook, TODOS refresh, and strict standards. MIGRATION-TODOS.md is now the primary daily navigation + agent handoff artifact (read it at start of every leg).
