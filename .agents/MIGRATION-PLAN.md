...
- Stage 8.8 (completed in PR #68): Finish the last 2 Personalizacion modules from the original 7 (T. Amb. Aplicable / tiposamb + Tipos Imp. Amb. / tiposimp, replacing final Placeholders) + Tipo Cursos for volume parity (3 full modules total), patch 0016, full modern pages + routes + verify extension + 8.8 playbook + docs. Branch `feat/stage-8.8-finish-personalizacion-remaining-modules`. No enhancements; follow established pattern and testing strategy.
- Stage 8.9 (in progress): Extract shared CatalogListado + CatalogFormulario bases to eliminate massive duplication across the simple catalog modules (Perfiles, Sedes, Clientes, Criterios + all Tipo* variants). Refactor ~10 modules to tiny extending classes while preserving exact behavior. Addresses "repetitive work" feedback. Branch `feat/stage-8.9-extract-catalog-base`. Plan committed first.
...
**Last Updated:** 2026-06. 8.9 (catalog base) complete on master. 9.0 (daily MIGRATION-TODOS list). 9.1: Criterios Ambientales menu hygiene (0019 + 0018 NOTE close). 9.2: Proveedores (0020, basic CRUD via catalog base). 9.3: Equipos (0021 table+seed, core fields CRUD with overrides, correct routes per clientes pattern) + hygiene fix for the 9.2 Proveedores list routes (were pointing to Formulario ShowPage instead of Listado). 9.4: Acciones de Mejora (0022 table+seed for acciones_mejora, Pages/Mejora with custom overrides for descripcion/fecha/cerrada + multi-fields, correct routes, templates, full verify/playbook + TODOS refresh). 9.5: Formación basic (Planes via plan_formacion) + Documentación shell (documentos landing + basic list); 0023/0024, Pages + templates for both, routes, verify + playbook + TODOS update. 9.6: Auditorías basic (programa_auditoria); 0025, Pages/Auditorias + templates, routes, verify + playbook + TODOS update. 9.7: Aspectos Ambientales basic (core `aspectos`); 0026, Pages/Aspectos + templates (shell list+form), routes, verify + playbook + TODOS update. 9.8: Cross-cut base extraction — enhanced CatalogListado + CatalogFormulario with protected helpers (getDb, loadItem, getPostData/validate/persist, build vars, etc.) to slash duplication in rich modules. Plan-first, verify, docs. 9.9: Indicadores basic (core `indicadores`); 0027, Pages/Indicadores (using 9.8 helpers), templates, routes, verify + playbook + TODOS update. 9.10: Procesos basic (core `procesos`); 0028, Pages/Procesos (using 9.8 helpers), templates, routes, verify + playbook + TODOS update. 9.11: Procesos deeper (Árbol + contenido); 0029, Pages/Procesos/Arbol modern tree view with hierarchy + contenido summaries, routes, verify + playbook + TODOS update. 9.12: Documentación tree first slice; modern Arbol view shell (grouped), routes, verify + playbook + TODOS update. 9.13: Cross-cut tree helpers (first delivery); helpers in CatalogListado (resolveParentNames etc.), refactored the two Arbols, verify + playbook + TODOS update. 9.14: Aspectos matrix first slice; modern grouped matrix view (Matriz page + template), route, verify + playbook + TODOS update. 9.15: Cross-cut filters + relations helpers in Catalog* + demo; verify + playbook + TODOS update. 9.16: Cross-cut full tree base (CatalogTree + refactored Arbols); verify + playbook + TODOS update. All via plan-first on feature branch, full verify/playbook, TODOS refresh, and strict standards. MIGRATION-TODOS.md is now the primary daily navigation + agent handoff artifact (read it at start of every leg).
