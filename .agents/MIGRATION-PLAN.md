...
**Last Updated:** 2026-06 (Stage 9.0 on `feat/stage-9.0-migration-todos`). Catalog base extraction (8.9) complete. Created `.agents/MIGRATION-TODOS.md` — the primary daily-navigable todo list / backlog of *all remaining modules to migrate*. This is now the go-to artifact for picking reviewable legs and handing off to other agents. MIGRATION-PLAN remains the architecture + full history view; STAGE-CHECKLISTS remains the detailed playbooks + evidence. 8.9 marked complete in prior leg.

**For daily leg planning and agent handoff**: open `.agents/MIGRATION-TODOS.md` first. It has usage rules, current snapshot of the 14 modernized modules, grouped remaining modules (Documentación, Procesos, Proveedores, Equipos, Mejora, Formación, Auditorías, Indicadores, Aspectos Ambientales + cross-cuts), suggested next legs of appropriate size, per-item complexity/est/size/legacy accion hints, and the "recipe" for adding one more module while following the established discipline.
