<?php
// $Header: /home/cvs/qnovaunidoCVS/html2ps_v181/public_html/css.list-style-type.inc.php,v 1.1 2006-08-31 08:36:05 jmartinez Exp $
...
class CSSListStyleType extends CSSSubProperty
{
    ...
    function value2ps($value)
    {
        ...
        if ($value === LST_DECIMAL) {
            return "/string";
        }
        if ($value === LST_DECIMAL_LEADING_ZERO) {
            return "/string";
        }
        if ($value === LST_LOWER_ROMAN) {
            return "/string";
        }
        if ($value === LST_UPPER_ROMAN) {
            return "/string";
        }
        if ($value === LST_LOWER_LATIN) {
            return "/string";
        }
        if ($value === LST_UPPER_LATIN) {
            return "/string";
        }
