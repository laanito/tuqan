<?php
// $Header: /home/cvs/qnovaunidoCVS/html2ps_v181/public_html/css.inc.php,v 1.1 2006-08-31 08:36:05 jmartinez Exp $
...
class CSSProperty
{
    ...
    function get()
    {
        return $this->_stack[0][0];
    }

    function inherit()
    {
        $this->push(($this->_inheritable ? $this->get() : $this->default_value()), false);
    }

    function inherit_text()
    {
        $this->push(($this->_inheritable_text ? $this->get() : $this->default_value()), false);
    }
    ...
    function push_default()
    {
        $this->push($this->default_value(), false);
    }

    function push_css($value)
    {
        $this->push_default();
        $this->css($value);
    }

    function push($value, $calculated = true)
    {
        array_unshift($this->_stack, array($value, $calculated));
    }

    //  function replace($value) { $this->pop(); $this->push($value, true); }
    function replace($value)
    {
        $this->_stack[0] = array($value, true);
    }
}
