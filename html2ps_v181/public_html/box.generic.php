<?php
// $Header: /home/cvs/qnovaunidoCVS/html2ps_v181/public_html/box.generic.php,v 1.1 2006-08-31 08:36:05 jmartinez Exp $
...
class GenericBox
{
    ...
    function _calc_auto_width_margins_float(&$parent)
    {
        ...
        if ($this->margin->left->auto) {
            $this->margin->left->value = 0;
        }
        if ($this->margin->right->auto) {
            $this->margin->right->value = 0;
        }
    }
...
//     };
    ...
    function _get_containing_block_absolute()
    {
        ...
        if (is_null($parent)) {
            die("No containing block found for absolute-positioned element");
        };
        ...
        while ((!is_null($parent->parent)) &&
            ($parent->position === POSITION_STATIC)) {
    ...
    {
        return $this->margin->right->value;
    }
    ...
    {
        return $this->padding->right->value;
    }

    function get_padding_left()
    {
        return $this->padding->left->value;
    }
    ...
    {
        return $this->padding->bottom->value;
    }

    function get_left_border()
    {
        return $this->get_left() - $this->padding->left->value - $this->border->left->get_width();
    }
    ...
    {
        return $this->get_top_padding() + $this->border->top->get_width();
    }
    ...
    {
        return $this->get_left() - $this->padding->left->value;
    }

    function get_right_padding()
    {
        return $this->get_left() + $this->get_width() + $this->padding->right->value;
    }

    function get_top_padding()
    {
        return $this->get_top() + $this->padding->top->value;
    }

    function get_bottom_padding()
    {
        return $this->get_bottom() - $this->padding->bottom->value;
    }
    ...
    {
        return $this->get_top_border() + $this->margin->top->value;
    }

    function put_top($value)
    {
        $this->_top = $value + $this->baseline_offset();
    }
    ...
    function _get_last_box()
    {
        return $this->_last;
    }

    function put_last(&$box)
    {
        $this->_last =& $box;
    }
