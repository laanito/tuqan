...
function initSubMenus(inputObj, initOffsetLeft, currentDepth) {
    ...
    if (subUl.length > 0) {
        ...
        ul.style.left = (leftPos - 18) + 'px';
        ul.style.top = (topPos - 151) + 'px';
...
function initDhtmlGoodiesMenu() {
    dhtmlgoodies_menuObj = document.getElementById('submenu');
    ...
    for (var no = 0; no < aTags.length; no++) {
        ...
        if (subUl.length > 0 && aTags[no].parentNode.parentNode.parentNode.id != 'submenu') {
