...
JSDragDropTree.prototype = {
    ...
    */
    ...
    , saveTree: function (initObj, saveString) {
        ...
        if (initObj.id == this.idOfTree) {
            ...
            this.ajaxObjects[ajaxIndex].onCompletion = function () {
                JSTreeObj.saveComplete(ajaxIndex);
    ...
    deleteItem: function (obj1, obj2) {
        var message = 'Pulse OK para borrar el elemento ' + obj2.innerHTML;
    ...
    initTree: function () {
        ...
        if (this.deleteAllowed || this.renameAllowed) {
            try {
                ...
                if (this.deleteAllowed) menuModel.addItem(1, 'Borrar', '', '', false, 'JSTreeObj.deleteItem');
                if (this.renameAllowed) menuModel.addItem(2, 'Renombrar', '', '', false, 'JSTreeObj.renameItem');
                menuModel.init();

                var menuModelRenameOnly = new DHTMLGoodies_menuModel();
                if (this.renameAllowed) menuModelRenameOnly.addItem(3, 'Renombrar', '', '', false, 'JSTreeObj.renameItem');
                menuModelRenameOnly.init();

                var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
                if (this.deleteAllowed) menuModelDeleteOnly.addItem(4, 'Borrar', '', '', false, 'JSTreeObj.deleteItem');
