...
class Formulario extends CatalogFormulario
{
    ...
    protected string $listRoute    = '/admin/formacion';
    ...
    public function ShowPage($id = null)
    {
        ...
        if ($id > 0) {
            ...
            $sql = "SELECT id, nombre, vigente, descripcion, activo, calidad, medioambiente FROM {$this->table} WHERE id = ?";
        ...
        $variables = [
            ...
            'isEdit'      => (bool)$item,
            'pageTitle'   => $item ? "Editar {$this->title}" : "Nuevo Plan de Formación",
    ...
    public function Procesar($id = null)
    {
        ...
        if ($id > 0) {
            $db->consultaPreparada(
                "UPDATE {$this->table} SET nombre = ?, vigente = ?, descripcion = ?, activo = ?, calidad = ?, medioambiente = ? WHERE id = ?",
                [$nombre, $vigente, $descripcion, $activo, $calidad, $medioambiente, $id]
            );
            $msg = $this->getSuccessMessage(true);
        } else {
            $db->consultaPreparada(
                "INSERT INTO {$this->table} (nombre, vigente, descripcion, activo, calidad, medioambiente) VALUES (?, ?, ?, ?, ?, ?)",
                [$nombre, $vigente, $descripcion, $activo, $calidad, $medioambiente]
            );
